...
sub setup {
	...
	if ($response == 20) {
		...
		my @params = ("https://api.imgur.com/oauth2/token", 'Content' => [%params]);
...
sub upload {
	...
	eval {
		...
		my %params = ('image' => $encoded_image,);

		my @params = ("https://api.imgur.com/3/image", 'Content' => [%params]);
		...
		if ($json_rsp->{'status'} ne 200) {
			...
			$self->{_links}{'status'} .=
				$d->get("Maybe you or Imgur revoked or expired an access token. Please close this dialog and try again. Your account will be re-authenticated the next time you upload a file.");
			return %{$self->{_links}};
		}
