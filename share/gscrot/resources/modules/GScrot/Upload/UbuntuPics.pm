...
package GScrot::Upload::UbuntuPics;
...
sub upload
{
	...
	if($username ne "" && $password ne ""){
		...
		unless(is_success($self->{_http_status})){
			$self->{_links}{'status'} = $self->{_http_status}; return %{$self->{_links}};
		}
...
sub show {
	...
	$upload_hbox->pack_start( Gtk2::Image->new_from_pixbuf( Gtk2::Gdk::Pixbuf->new_from_file_at_scale( "$self->{_gscrot_root}/share/gscrot/resources/icons/logo-ubuntu-pics.png", 100, 100, TRUE ) ), TRUE, TRUE, 0 );
