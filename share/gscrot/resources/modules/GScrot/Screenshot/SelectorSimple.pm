...
sub select_simple {
	...
	$scwin->set_size_request( 100 * $self->{_zoom_size_factor},
							  100 * $self->{_zoom_size_factor} );
	...
	$root_item =
		Gnome2::Canvas::Item->new(
						 ...
						 pixbuf =>
							 ...
							 $self->{ _root }, undef, 0, 0, 0, 0, $self->{ _root }->{ w }, $self->{ _root }->{ h }
																 ),
								 );
	...
	if ( Gtk2::Gdk->pointer_is_grabbed ) {
		...
		my ( $rx, $ry, $rw, $rh, $rect_x, $rect_y, $rect_w, $rect_h ) =
			( 0, 0, 0, 0, 0, 0, 0, 0 );
				...
				if ( $event->type eq 'key-press' ) {
					if ( $event->keyval == $Gtk2::Gdk::Keysyms{ Escape } ) {
						if ( $rect_w > 1 ) {
							...
							$self->{_root}->draw_rectangle( $gc, 0, $rect_x, $rect_y,
												   $rect_w, $rect_h );
						}
						...
						$self->ungrab_pointer_and_keyboard(
										!$self->{_zoom_active}, TRUE, TRUE );
					...
					$self->ungrab_pointer_and_keyboard( !$self->{_zoom_active},
													  TRUE, TRUE );
					...
					if ( $rect_w > 1 ) {
						...
						$output =
							$self->get_pixbuf_from_drawable( $self->{ _root }, $rect_x,
														$rect_y,
														$rect_w + 1,
														$rect_h + 1,
														$self->{_include_cursor}, $self->{_delay} );
				...
				#check pos and geometry of the zoom window and move it if needed
					   ...
					   )
					{
						...
						if ( $zoom_window_init ) {
							...
							$zoom_window->move( 0,
										   $self->{_root}->{h} - $zoom_window_height );
