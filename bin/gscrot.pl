...
$menuitem_open->set_image(
	Gtk2::Image->new_from_icon_name( 'gtk-open', 'menu' ) );
$menuitem_open->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{O}, qw/control-mask/, qw/visible/ );
...
$menuitem_revert->set_image(
	Gtk2::Image->new_from_icon_name( 'gtk-revert-to-saved-ltr', 'menu' ) );
$menuitem_revert->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{Z}, qw/control-mask/, qw/visible/ );
...
$menuitem_save->set_image(
	Gtk2::Image->new_from_icon_name( 'gtk-save', 'menu' ) );
$menuitem_save->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{S}, qw/control-mask/, qw/visible/ );
...
$menuitem_quit->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{Q}, qw/control-mask/, qw/visible/ );
...
$menuitem_settings->set_image(
	Gtk2::Image->new_from_stock( 'gtk-properties', 'menu' ) );
$menuitem_settings->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{P}, qw/mod1-mask/, qw/visible/ );
...
$menuitem_selection->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{S}, qw/mod1-mask/, qw/visible/ );
...
$menuitem_raw->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{F}, qw/mod1-mask/, qw/visible/ );
...
$menuitem_window->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{W}, qw/mod1-mask/, qw/visible/ );
...
$menuitem_window_sect->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{W}, qw/shift-mask/, qw/visible/ );
...
$menuitem_first->set_image(
	Gtk2::Image->new_from_stock( 'gtk-goto-first', 'menu' ) );
$menuitem_first->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{F5}, qw/control-mask/, qw/visible/ );
...
$menuitem_back->set_image(
	Gtk2::Image->new_from_stock( 'gtk-go-back', 'menu' ) );
$menuitem_back->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{F6}, qw/control-mask/, qw/visible/ );
...
$menuitem_forward->set_image(
	Gtk2::Image->new_from_stock( 'gtk-go-forward', 'menu' ) );
$menuitem_forward->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{F7}, qw/control-mask/, qw/visible/ );
...
$menuitem_last->set_image(
	Gtk2::Image->new_from_stock( 'gtk-goto-last', 'menu' ) );
$menuitem_last->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{F8}, qw/control-mask/, qw/visible/ );
...
$menuitem_question->set_image(
	Gtk2::Image->new_from_pixbuf(
		Gtk2::Gdk::Pixbuf->new_from_file_at_size(
			"$gscrot_root/share/gscrot/resources/icons/help.svg",
			Gtk2::IconSize->lookup('menu')
		)
	)
);
$menuitem_question->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{A}, qw/control-mask/, qw/visible/ );
...
$menuitem_bug->set_image(
	Gtk2::Image->new_from_pixbuf(
		Gtk2::Gdk::Pixbuf->new_from_file_at_size(
			"$gscrot_root/share/gscrot/resources/icons/aiutare.svg",
			Gtk2::IconSize->lookup('menu')
		)
	)
);
$menuitem_bug->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{B}, qw/control-mask/, qw/visible/ );
$menu_help->append($menuitem_bug);
$menuitem_bug->signal_connect( "activate", \&evt_bug, $window );

my $menuitem_translate =
  Gtk2::ImageMenuItem->new( $d->get("Add a translation") );
$menuitem_translate->set_image(
	Gtk2::Image->new_from_pixbuf(
		Gtk2::Gdk::Pixbuf->new_from_file_at_size(
			"$gscrot_root/share/gscrot/resources/icons/translate.svg",
			Gtk2::IconSize->lookup('menu')
		)
	)
);
$menuitem_translate->add_accelerator( "activate", $accel_group,
	$Gtk2::Gdk::Keysyms{T}, qw/control-mask/, qw/visible/ );
...
$button_raw_menu->set_arrow_tooltip( Gtk2::Tooltips->new,
	$d->get("Capture a specific workspace"), 'arrow' );
...
$button_window_menu->set_arrow_tooltip( Gtk2::Tooltips->new,
	$d->get("Take a screenshot of a specific window"), 'arrow' );

#############BUTTON_WINDOW######################

#############BUTTON_WINDOW_SECTION######################
...
$tooltip_saveDir->set_tip( $saveDir_button,
	$d->get("Your screenshots will be saved\nto this directory") );
$tooltip_saveDir->set_tip( $saveDir_label,
	$d->get("Your screenshots will be saved\nto this directory") );
...
my $save_at_close_active =
  Gtk2::CheckButton->new_with_label( $d->get("Save settings when exiting") );
my $ask_quit_active = Gtk2::CheckButton->new_with_label(
	$d->get("Show \"Do you really want to quit?\" dialog when exiting") );
...
$tooltip_hide->set_tip( $hide_active,
	$d->get("Automatically hide GScrot Window when taking a screenshot") );
...
$tooltip_autocopy->set_tip( $autocopy_active,
	$d->get("Automatically copy screenshot to clipboard") );
...
$tooltip_close_at_close->set_tip( $close_at_close_active,
	$d->get("Minimize to tray when closing main window") );
...
$tooltip_save_at_close->set_tip( $save_at_close_active,
	$d->get("Save settings automatically when exiting GScrot") );

$ask_quit_active->signal_connect(
	'toggled' => \&evt_behavior_handle,
	'ask_quit_toggled'
);
if ( defined $settings_xml->{'general'}->{'ask_at_close'} ) {
	$ask_quit_active->set_active(
		$settings_xml->{'general'}->{'ask_at_close'} );
}
else {
...
$tooltip_ask_quit->set_tip( $ask_quit_active,
	$d->get("Show \"Do you really want to quit?\" dialog when exiting") );
...
foreach ( keys %gm_programs ) {
		...
		&& $gm_programs{$_}->{'name'} ne "" )
	{
		...
		$model->set( $model->append, 0, $pixbuf, 1, $gm_programs{$_}->{'name'},
			2, $_ );
	}
	else {
...
$tooltip_progname->set_tip( $progname,
	$d->get("Open your screenshot\nwith this program after capturing") );
$tooltip_progname->set_tip( $progname_active,
	$d->get("Open your screenshot\nwith this program after capturing") );
$tooltip_progname->set_tip( $progname_label,
	$d->get("Open your screenshot\nwith this program after capturing") );
...
$tooltip_im_colors->set_tip( $combobox_im_colors,
	$d->get("Automatically reduce colors \nafter taking a screenshot") );
$tooltip_im_colors->set_tip( $im_colors_active,
	$d->get("Automatically reduce colors \nafter taking a screenshot") );
$tooltip_im_colors->set_tip( $im_colors_label,
	$d->get("Automatically reduce colors \nafter taking a screenshot") );
...
$tooltip_border->set_tip( $combobox_border,
	$d->get("Include wm border when selecting a window") );
$tooltip_border->set_tip( $border_label,
	$d->get("Include wm border when selecting a window") );
...
#accounts
my $accounts_model = Gtk2::ListStore->new(
...
if ( keys(%plugins) > 0 ) {
	...
	foreach ( keys %plugins ) {
		if ( $plugins{$_}->{'binary'} ne "" ) {
			...
			$effects_model->set(
				$effects_model->append,     0, $pixbuf,                   1,
				$plugins{$_}->{'name'},     2, $plugins{$_}->{'binary'},  3,
				$plugins{$_}->{'category'}, 4, $plugins{$_}->{'tooltip'}, 5,
				$pixbuf_jpeg,               6, $pixbuf_png,               7,
				$plugins{$_}->{'lang'}
			);
		}
		else {
...
sub fct_init {
	...
	if ( @ARGV > 0 ) {
		...
		foreach $arg (@args) {
			if ( $arg eq "--debug" ) {
				...
				my @gscrot_pids = `pidof -o $$ -x gscrot`;
				foreach (@gscrot_pids) {
					kill USR2 => $_;
					die;
				}
	...
	if ( system("which gtklp") == 0 ) {
		...
		die
"ERROR: dependency is missing --> gtklp is not installed on your system!\n\n";
	}
	if ( system("which gnome-web-photo") == 0 ) {
		...
		die
"ERROR: dependency is missing --> gnome-web-photo is not installed on your system!\n\n";
	}
...
"\nINFO: using custom program settings found at $ENV{ 'HOME' }/.gscrot/programs.conf\n";
	...
	foreach (@plugins) {
		if ( -d $_ ) {
			...
			$plugins{$_}->{'pixmap'} = "$_/$dir_name.png"
...
sub evt_take_screenshot {
	...
	my $current_counter =
	  sprintf( "%02d", scalar( keys %session_screens ) + 1 );
	...
	if ( $data eq "raw" || $data eq "tray_raw" ) {
		...
		$screenshot = &fct_gscrot_workspace(
			$folder,         $filename_value,
			$filetype_value, $quality_value,
		...
		|| $data eq "tray_section" )
	{
		...
		$screenshot =
		  &fct_gscrot_window( $folder, $filename_value, $filetype_value,
			$quality_value, $delay_value, $combobox_border->get_active, $data );
...
`gnome-web-photo --timeout=$timeout --mode=photo --format=$filetype_value -q $quality_value '$url' '$folder/$filename_value.$filetype_value'`;
		...
		$screenshot =
		  &fct_gscrot_window_direct( $folder, $filename_value, $filetype_value,
			$quality_value, $delay_value, $combobox_border->get_active, $xid );
		...
		$screenshot =
		  &fct_gscrot_workspace( $folder, $filename_value, $filetype_value,
			$quality_value, $delay_value, $combobox_border->get_active,
			$selected_workspace );
...
"Screenshot failed!\nMaybe mouse pointer could not be grabbed or the selected area is invalid."
		...
		if ( $thumbnail_active->get_active ) {
			...
			my $theight =
			  int(
				$screenshot_thumbnail->Get('rows') * ( $thumbnail_value / 100 )
			  );
...
"Screenshot failed!\nMaybe mouse pointer could not be grabbed or the selected area is invalid."
	...
	#	GScrot::Resize::fct_execute_plugin();
...
sub evt_delete_window {
	...
	my $exit_image =
	  Gtk2::Image->new_from_icon_name( 'gtk-dialog-question', 'dialog' );
	my $exit_label = Gtk2::Label->new();
	$exit_label->set_text( $d->get("Do you really want to quit GScrot?") );
	my $ask_active = Gtk2::CheckButton->new_with_label(
		$d->get("Do not ask this question again") );
	...
	if ( $ask_active->get_active ) {
		...
		if ( $found == FALSE ) {
			open( FILE, ">>$ENV{ HOME }/.gscrot/settings.conf" )
			  or
			  &dlg_status_message( 1, $d->get("Settings could not be saved") );
...
sub fct_integrate_screenshot_in_notebook {
	...
	$session_screens{$key}->{'tab_icon'} =
	  Gtk2::Image->new_from_pixbuf( $session_screens{$key}->{'thumb'}
		  ->scale_simple( Gtk2::IconSize->lookup('menu'), 'bilinear' ) );
	...
	my $new_index =
	  $notebook->append_page( fct_create_tab( $key, FALSE ), $hbox_tab_label );
...
sub fct_create_tab {
	...
	my $image_remove = Gtk2::Image->new_from_icon_name( 'gtk-close', 'menu' );
	my $button_remove = Gtk2::ToolButton->new( $image_remove, undef );
	...
	my $image_delete = Gtk2::Image->new_from_icon_name( 'gtk-delete', 'menu' );
	my $button_delete = Gtk2::ToolButton->new( $image_delete, undef );
	...
	my $image_clipboard = Gtk2::Image->new_from_icon_name( 'gtk-copy', 'menu' );
	my $button_clipboard = Gtk2::ToolButton->new( $image_clipboard, undef );
	$button_clipboard->signal_connect(
		clicked => \&evt_in_tab,
		'clipboard' . $key
	);
	...
	my $image_reopen =
	  Gtk2::Image->new_from_icon_name( 'gtk-redo-ltr', 'menu' );
	my $button_reopen = Gtk2::ToolButton->new( $image_reopen, undef );
	...
	my $image_upload = Gtk2::Image->new_from_icon_name( 'gtk-go-up', 'menu' );
	my $button_upload = Gtk2::ToolButton->new( $image_upload, undef );
	...
	my $image_rename = Gtk2::Image->new_from_icon_name( 'gtk-edit', 'menu' );
	my $button_rename = Gtk2::ToolButton->new( $image_rename, undef );
	...
	my $image_plugin = Gtk2::Image->new_from_icon_name( 'gtk-execute', 'menu' );
	my $button_plugin = Gtk2::ToolButton->new( $image_plugin, undef );
	...
	my $button_draw = Gtk2::ToolButton->new( $image_draw, undef );
	...
	my $image_print = Gtk2::Image->new_from_icon_name( 'gtk-print', 'menu' );
	my $button_print = Gtk2::ToolButton->new( $image_print, undef );
	...
	unless ($is_all) {
		...
		$session_screens{$key}->{'tooltip_filename_tab'}->set_tip(
			$session_screens{$key}->{'filename_label'},
...
"$gscrot_root/share/gscrot/resources/icons/Image-missing.svg",
		...
		$vbox_fileinfos->pack_start( $session_screens{$key}->{'filename_label'},
			FALSE, TRUE, 2 );
		...
		$vbox_fileinfos->pack_start( $session_screens{$key}->{'geometry_label'},
			FALSE, TRUE, 2 );
	...
	if ( defined $widget ) {
		...
		if ( $old_width != $image->{'current_width'} ) {
			$image->set_from_pixbuf(
				$thumb->scale_simple(
					$image->{'current_width'}, $image->{'current_height'},
					'bilinear'
				)
			);
		}
...
sub evt_show_settings {
	...
	if ( $settings_dialog_response eq "accept" ) {
		&fct_save_settings;
...
sub evt_in_tab {
	...
	if ( $key =~ m/^delete\[/ ) {
		...
		&dlg_status_message( 1,
			$session_screens{$key}->{'filename'} . " " . $d->get("deleted") )
	...
	if ( $key =~ m/^rename\[/ ) {
		...
		&dlg_status_message( 1,
			$session_screens{$key}->{'filename'} . " " . $d->get("renamed") )
	...
	if ( $key =~ m/^clipboard\[/ ) {
		...
		my $pixbuf =
		  Gtk2::Gdk::Pixbuf->new_from_file( $session_screens{$key}->{'long'} );
		$clipboard->set_image($pixbuf);
		&dlg_status_message( 1,
			    $session_screens{$key}->{'filename'} . " "
			...
			&& -w "$ENV{ HOME }/.gscrot/settings.xml" )
		{
			if (
				&dlg_question_message(
					$d->get("Do you want to overwrite the existing settings?")
				)
			  )
			{    #ask is settings-file exists
				&fct_save_settings;
			}
		}
		else {
...
sub fct_save_settings {
	...
	if ( $save_part eq "bindings" ) {
		...
		if ( $keybinding_active->get_active() ) {
			...
			$client->set( $shortcut_full,
				{ type => 'string', value => $capture_key->get_text(), } );
		}
		else {
			$client->set( $command_full,
		...
		if ( $keybinding_sel_active->get_active() ) {
			$client->set( $command_sel,
				{ type => 'string', value => "$gscrot_path --window", } );
			$client->set( $shortcut_sel,
				{ type => 'string', value => $capture_sel_key->get_text(), } );
		}
		else {
			$client->set( $command_sel,
				{ type => 'string', value => 'gnome-screenshot --window', } );
			$client->set( $shortcut_sel,
		...
		$settings{'general'}->{'zoom_size'} = 1 if ( $zoom_size1->get_active );
		$settings{'general'}->{'zoom_size'} = 2 if ( $zoom_size2->get_active );
		$settings{'general'}->{'zoom_size'} = 3 if ( $zoom_size3->get_active );
	...
	if ( $input_response eq "accept" ) {
		...
		{    #filenames eq? -> nothing to do here
			unless ( &fct_file_exists($dlg_rename_text) ) {
				rename(
					&fct_switch_home_in_file($old_file_name_full),
	...
	if ( $plugin_response eq "accept" ) {
		...
		if ( $plugin_lang eq "perl" ) {
		   ...
		   #initiate the socket to draw the contents of the plugin to our dialog
			my $plugin_socket =
			  Gtk2::Dialog->new( $plugin_name, $window,
				[qw/modal destroy-with-parent/] );
			...
			if ( $pid == 0 ) {
				exec(
					sprintf(
"$^X $plugin_value %d $dlg_plugin_text $session_screens{$key}->{'width'} $session_screens{$key}->{'height'} $session_screens{$key}->{'filetype'}\n",
						$socket->get_id )
				);
			}
		 ...
		 #...if not => simple execute the plugin via system (e.g. shell plugins)
			...
			print
"$plugin_value $dlg_plugin_text $session_screens{$key}->{'width'} $session_screens{$key}->{'height'} $session_screens{$key}->{'filetype'} submitted to plugin\n"
			  if $debug_cparam;
			if (
				system(
"'$plugin_value' '$dlg_plugin_text' '$session_screens{$key}->{'width'}' '$session_screens{$key}->{'height'}' '$session_screens{$key}->{'filetype'}' "
				) == 0
			  )
			{
				&dlg_status_message( 1,
...
sub dlg_account_chooser_and_upload {
	...
	foreach ( keys %accounts ) {
		$model->set(
			...
			$accounts{$_}->{'username'}, 2, $accounts{$_}->{'password'}
		  )
...
sub fct_update_first_tab {
	...
	if ( keys(%session_screens) == 0 ) {
		...
		$session_start_screen{'first_page'}->{'btn_reopen'}
		  ->set_sensitive(FALSE);
		$session_start_screen{'first_page'}->{'btn_print'}
		  ->set_sensitive(FALSE);
	}
	else {
		$session_start_screen{'first_page'}->{'btn_remove'}
...
sub fct_update_tab {
	  ...
	  )
	{
	   ...
	   #update tab icon - maybe pic changed due to use of plugin or drawing tool
		...
		$session_screens{$key}->{'tab_icon'}->set_from_pixbuf(
			Gtk2::Gdk::Pixbuf->new_from_file_at_size(
				"$gscrot_root/share/gscrot/resources/icons/Image-missing.svg",
				Gtk2::IconSize->lookup('menu')
			)
		);
		$session_screens{$key}->{'image'}->set_from_pixbuf(
			Gtk2::Gdk::Pixbuf->new_from_file_at_size(
				"$gscrot_root/share/gscrot/resources/icons/Image-missing.svg",
				Gtk2::IconSize->lookup('dialog')
			)
...
sub fct_resize_imagemagick_object {
	...
	$operator = $combobox_thumb_operator->get_active unless defined $operator;
					...
					if ( defined $children{'last_win'} ) {
						...
						$children{'curr_win'}{'height'} -=
						  ...
						  if ( $children{'curr_win'}{'y'} +
							$children{'curr_win'}{'height'} > $rootheightp );
...
sub fct_gscrot_window_direct {
	...
	#get the pixbuf from drawable and save the file
	#maybe window is partially not on the screen
	$widthp -= $xp + $widthp - $rootwidthp if ( $xp + $widthp > $rootwidthp );
...
sub fct_gscrot_workspace {
	...
	my ( $folder, $filename_value, $filetype_value, $quality_value,
		$delay_value, $border, $selected_workspace )
	  = @_;
...
sub fct_ret_workspace_menu {
	...
	if ( Gtk2->CHECK_VERSION( 2, 11, 0 ) ) {
		...
		for (
			my $wcount = 0 ;
			$wcount < $screen->get_workspace_count ;
			$wcount++
		  )
		{
