...
my $menuitem_session =
	Gtk2::MenuItem->new_with_mnemonic( $d->get( "_Session" ) );
...
$button_web_menu->set_arrow_tooltip(
									 Gtk2::Tooltips->new,
									 $d->get(
										 "Set up a connection timeout (seconds)"
									 ),
									 'arrow'
								   );
...
sub evt_take_screenshot {
	...
	$current_counter = sprintf( "%02d", $current_counter );
...
sub fct_integrate_screenshot_in_notebook {
	...
	$notebook->set_tab_detachable( $session_screens{ $key }->{ 'tab_child' },
								   TRUE );
...
sub fct_save_settings {
	...
	#plugins
	foreach ( keys %plugins ) {
		$settings{ 'plugins' }->{ $_ }->{ 'menu' } = $plugins{ $_ }->{ 'menu' }
...
sub fct_update_first_tab {
	...
	foreach ( sort keys %session_screens ) {
		$session_start_screen{ 'first_page' }->{ 'model' }->set(
					 ...
					 0,
					 $session_screens{ $_ }->{ 'thumb' }
						 ->scale_simple( 48, 48, 'tiles' ),
					 1,
		...
		Gtk2::Gdk::Event->handler_set(
			sub {
				...
				if ( $event->type eq 'key-press' ) {
					...
					Gtk2::Gdk::X11->ungrab_server unless $window_selected;
...
sub dlg_upload_links_imagebanana {
	...
	$upload_vbox->pack_start( $upload_hbox, TRUE, TRUE, 10 );
	$upload_vbox->pack_start(
							  Gtk2::Label->new(
											 $d->get( "Thumbnail for websites" )
							  ),
							  TRUE, TRUE, 2
							);
	...
	$upload_vbox->pack_start(
							  Gtk2::Label->new(
												$d->get( "Popup for websites" )
											  ),
							  TRUE, TRUE, 2
							);
	$upload_vbox->pack_start( $entry_popup, TRUE, TRUE, 2 );
	$upload_vbox->pack_start( Gtk2::Label->new( $d->get( "Direct link" ) ),
							  TRUE, TRUE, 2 );
	$upload_vbox->pack_start( $entry_direct, TRUE, TRUE, 2 );
	$upload_vbox->pack_start(
							  Gtk2::Label->new(
											   $d->get( "Hotlink for websites" )
											  ),
							  TRUE, TRUE, 2
							);
