...
#settings
$settings_dialog->vbox->add( $notebook_settings );
...
$menu_file->append( $menuitem_revert );
$menuitem_revert->signal_connect( "activate", \&evt_settings, 'menu_revert' );
...
$menuitem_save->signal_connect( "activate", \&evt_settings, 'menu_save' );
...
$menuitem_window_sect->add_accelerator( "activate", $accel_group, $Gtk2::Gdk::Keysyms{ W }, qw/shift-mask/, qw/visible/ );
...
$ask_quit_active->signal_connect( 'toggled' => \&evt_behavior_handle,
								  'ask_quit_toggled' );

if ( defined $settings_xml->{ 'general' }->{ 'ask_at_close' } ) {
   $ask_quit_active->set_active( $settings_xml->{ 'general' }->{ 'ask_at_close' } );
}
else {
   $ask_quit_active->set_active( TRUE );
}
...
$behavior_vbox->pack_start( $autocopy_active,       FALSE, TRUE, 5 );
...
sub evt_behavior_handle {
   ...
   if ( $data eq "close_at_close_toggled" ) {
	  $ask_quit_active->set_sensitive( FALSE )
		  if $close_at_close_active->get_active;
	  $ask_quit_active->set_sensitive( TRUE )
		  unless $close_at_close_active->get_active;
   }
...
sub evt_delete_window {
   ...
   if ( $data eq "menu_quit" && $save_at_close_active->get_active ) {
	  if (    -f "$ENV{ HOME }/.gscrot/settings.xml"
		   && -w "$ENV{ HOME }/.gscrot/settings.xml" )
	  {
		 &fct_save_settings;
	  }
   }
   ...
   my $dlg_header = $d->get( "Quit GScrot" );
   ...
   if ( $ask_active->get_active ) {
	  open( FILE, ">>$ENV{ HOME }/.gscrot/settings.conf" )
		  or &dlg_status_message( 1, $d->get( "Settings could not be saved" ) );
	  my @saved_lines = <FILE>;
	  close( FILE )
		  or &dlg_status_message( 1, $d->get( "Settings could not be saved" ) );
	  my $found = FALSE;
...
sub evt_settings {
   ...
   if ( $data eq "menu_save" ) {
	  if (    -f "$ENV{ HOME }/.gscrot/settings.xml"
		   && -w "$ENV{ HOME }/.gscrot/settings.xml" )
	  {
		 if ( &dlg_question_message( $d->get( "Do you want to overwrite the existing settings?" ) ) ) {    #ask is settings-file exists
			&fct_save_settings;
		 }
	  }
	  else {
		 &fct_save_settings;                                                                               #do it directly if not
	  }
   }
   elsif ( $data eq "menu_revert" ) {
	  if (    -f "$ENV{ HOME }/.gscrot/settings.xml"
		   && -r "$ENV{ HOME }/.gscrot/settings.xml" )
	  {
		 &fct_load_settings;
	  }
	  else {
		 &dlg_info_message( $d->get( "There are no stored settings" ) );
	  }
   }
...
sub fct_save_settings {
   ...
   if ( $save_part eq "bindings" ) {
	  ...
	  open( SETTFILE, ">$ENV{ HOME }/.gscrot/settings.xml" )
	  ...
	  open( ACC_FILE, ">$ENV{ HOME }/.gscrot/accounts.xml" )
...
sub fct_load_settings {
   my ( $widget, $data ) = @_;
   $settings_xml = undef;
   if ( &fct_file_exists( "$ENV{ HOME }/.gscrot/settings.xml" ) ) {
	  $settings_xml = XMLin( "$ENV{ HOME }/.gscrot/settings.xml" );
	  if ( $data eq 'menu_revert' ) {
		 $combobox_type->set_active( $settings_xml->{ 'general' }->{ 'filetype' } );
		 $scale->set_value( $settings_xml->{ 'general' }->{ 'quality' } );
		 $filename->set_text( $settings_xml->{ 'general' }->{ 'filename' } );
		 $saveDir_button->set_current_folder( $settings_xml->{ 'general' }->{ 'folder' } );
...
sub fct_load_accounts {
   my $accounts_xml = XMLin( "$ENV{ HOME }/.gscrot/accounts.xml" )
	   if &fct_file_exists( "$ENV{ HOME }/.gscrot/accounts.xml" );
