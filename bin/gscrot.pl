...
&fct_check_installed_programs if keys( %gm_programs ) > 0;
my %plugins;        #hash to store plugin infos
&fct_check_installed_plugins if keys( %plugins ) > 0;
...
$SIG{ USR2 } = sub { &evt_take_screenshot( 'global_keybinding', 'select' ) };
...
$window->set_default_icon( Gtk2::Gdk::Pixbuf->new_from_file_at_size( "$gscrot_root/share/pixmaps/gscrot.svg", 500, 500 ) );
...
my $settings_dialog =
	Gtk2::Dialog->new( GSCROT_NAME . " - " . $d->get( "Settings" ), $window, [ qw/modal destroy-with-parent/ ], 'gtk-close' => 'close' );
...
$menuitem_open->signal_connect( "activate", \&evt_settings, 'menu_open' );
...
my $menuitem_settings = Gtk2::ImageMenuItem->new( $d->get( "Settings" ) );
$menuitem_settings->set_image( Gtk2::Image->new_from_stock( 'gtk-properties', 'menu' ) );
$menuitem_settings->add_accelerator( "activate", $accel_group, $Gtk2::Gdk::Keysyms{ P }, qw/mod1-mask/, qw/visible/ );
...
$menuitem_window_sect->set_image(
								  Gtk2::Image->new_from_pixbuf(
											   Gtk2::Gdk::Pixbuf->new_from_file_at_size(
												  "$gscrot_root/share/gscrot/resources/icons/sel_window_section.svg", Gtk2::IconSize->lookup( 'menu' )
											   )
								  )
								);
$menuitem_window_sect->add_accelerator( "activate", $accel_group, $Gtk2::Gdk::Keysyms{ S }, qw/shift-mask/, qw/visible/ );
...
my $menuitem_first = Gtk2::ImageMenuItem->new( $d->get( "First Page" ) );
$menuitem_first->set_image( Gtk2::Image->new_from_stock( 'gtk-goto-first', 'menu' ) );
$menuitem_first->add_accelerator( "activate", $accel_group, $Gtk2::Gdk::Keysyms{ F5 }, qw/control-mask/, qw/visible/ );
$menu_session->append( $menuitem_first );
$menuitem_first->signal_connect( "activate", \&evt_navigate, 'first' );
my $menuitem_back = Gtk2::ImageMenuItem->new( $d->get( "Previous Page" ) );
$menuitem_back->set_image( Gtk2::Image->new_from_stock( 'gtk-go-back', 'menu' ) );
$menuitem_back->add_accelerator( "activate", $accel_group, $Gtk2::Gdk::Keysyms{ F6 }, qw/control-mask/, qw/visible/ );
$menu_session->append( $menuitem_back );
$menuitem_back->signal_connect( "activate", \&evt_navigate, 'previous' );
my $menuitem_forward = Gtk2::ImageMenuItem->new( $d->get( "Next Page" ) );
$menuitem_forward->set_image( Gtk2::Image->new_from_stock( 'gtk-go-forward', 'menu' ) );
$menuitem_forward->add_accelerator( "activate", $accel_group, $Gtk2::Gdk::Keysyms{ F7 }, qw/control-mask/, qw/visible/ );
$menu_session->append( $menuitem_forward );
$menuitem_forward->signal_connect( "activate", \&evt_navigate, 'forward' );
my $menuitem_last = Gtk2::ImageMenuItem->new( $d->get( "Last Page" ) );
$menuitem_last->set_image( Gtk2::Image->new_from_stock( 'gtk-goto-last', 'menu' ) );
$menuitem_last->add_accelerator( "activate", $accel_group, $Gtk2::Gdk::Keysyms{ F8 }, qw/control-mask/, qw/visible/ );
$menu_session->append( $menuitem_last );
$menuitem_last->signal_connect( "activate", \&evt_navigate, 'last' );
my $menuitem_session = Gtk2::MenuItem->new_with_mnemonic( $d->get( "_Session" ) );
...
$menuitem_question->set_image(
				 Gtk2::Image->new_from_pixbuf(
					Gtk2::Gdk::Pixbuf->new_from_file_at_size( "$gscrot_root/share/gscrot/resources/icons/help.svg", Gtk2::IconSize->lookup( 'menu' ) )
				 )
);
...
$menuitem_bug->set_image(
			  Gtk2::Image->new_from_pixbuf(
				 Gtk2::Gdk::Pixbuf->new_from_file_at_size( "$gscrot_root/share/gscrot/resources/icons/aiutare.svg", Gtk2::IconSize->lookup( 'menu' ) )
			  )
);
...
$menuitem_translate->set_image(
			Gtk2::Image->new_from_pixbuf(
			   Gtk2::Gdk::Pixbuf->new_from_file_at_size( "$gscrot_root/share/gscrot/resources/icons/translate.svg", Gtk2::IconSize->lookup( 'menu' ) )
			)
);
...
my $image_select =
	Gtk2::Image->new_from_pixbuf(
   Gtk2::Gdk::Pixbuf->new_from_file_at_size( "$gscrot_root/share/gscrot/resources/icons/selection.svg", Gtk2::IconSize->lookup( 'large-toolbar' ) ) );
my $button_select_menu = Gtk2::MenuToolButton->new( $image_select, $d->get( "Selection" ) );
...
$tooltip_select->set_tip( $button_select_menu, $d->get( "Draw a rectangular capture area with your mouse\nto select a specified screen area" ) );
...
my $image_raw =
	Gtk2::Image->new_from_pixbuf(
	 Gtk2::Gdk::Pixbuf->new_from_file_at_size( "$gscrot_root/share/gscrot/resources/icons/fullscreen.svg", Gtk2::IconSize->lookup( 'large-toolbar' ) )
	);
my $button_raw_menu = Gtk2::MenuToolButton->new( $image_raw, $d->get( "Fullscreen" ) );
...
$tooltip_raw->set_tip( $button_raw_menu, $d->get( "Take a screenshot of your whole desktop" ) );
$button_raw_menu->set_arrow_tooltip( Gtk2::Tooltips->new, $d->get( "Capture a specific workspace" ), 'arrow' );
...
my $image_window =
	Gtk2::Image->new_from_pixbuf(
	 Gtk2::Gdk::Pixbuf->new_from_file_at_size( "$gscrot_root/share/gscrot/resources/icons/sel_window.svg", Gtk2::IconSize->lookup( 'large-toolbar' ) )
	);
my $button_window_menu = Gtk2::MenuToolButton->new( $image_window, $d->get( "Window" ) );
...
$tooltip_window->set_tip( $button_window_menu, $d->get( "Select a window with your mouse" ) );
$button_window_menu->set_arrow_tooltip( Gtk2::Tooltips->new, $d->get( "Take a screenshot of a specific window" ), 'arrow' );
...
my $button_window_sect = Gtk2::ToolButton->new( $image_window_sect, $d->get( "Section" ) );
...
my $image_web =
	Gtk2::Image->new_from_pixbuf(
   Gtk2::Gdk::Pixbuf->new_from_file_at_size( "$gscrot_root/share/gscrot/resources/icons/web_image.svg", Gtk2::IconSize->lookup( 'large-toolbar' ) ) );
...
$tooltip_web->set_tip( $button_web_menu, $d->get( "Take a screenshot of a website" ) );
$button_web_menu->set_arrow_tooltip( Gtk2::Tooltips->new, $d->get( "Set up a connection timeout (seconds)" ), 'arrow' );
...
#trayicon
#--------------------------------------
my $icon = Gtk2::Image->new_from_pixbuf( Gtk2::Gdk::Pixbuf->new_from_file_at_size( "$gscrot_root/share/pixmaps/gscrot.svg", 24, 24 ) );
...
foreach ( sort @saved_profiles ) {
   ...
   $combobox_settings_profiles->set_active( $current_index ) if $settings_xml->{ 'general' }->{ 'last_profile_name' } eq $last_profile_name;
   $current_index++;
}
my $tooltip_profiles = Gtk2::Tooltips->new;
$tooltip_profiles->set_tip( $combobox_settings_profiles, $d->get( "Choose a profile" ) );


my $button_profile_save = Gtk2::Button->new;
$button_profile_save->signal_connect( 'clicked', \&evt_save_profile, $combobox_settings_profiles );
$button_profile_save->set_image( Gtk2::Image->new_from_stock( 'gtk-save', 'button' ) );
my $tooltip_profiles_save = Gtk2::Tooltips->new;
$tooltip_profiles_save->set_tip( $button_profile_save, $d->get( "Save configuration as profile" ) );

my $button_profile_delete = Gtk2::Button->new;
$button_profile_delete->signal_connect( 'clicked', \&evt_delete_profile, $combobox_settings_profiles );
$button_profile_delete->set_image( Gtk2::Image->new_from_stock( 'gtk-delete', 'button' ) );
my $tooltip_profiles_delete = Gtk2::Tooltips->new;
$tooltip_profiles_delete->set_tip( $button_profile_delete, $d->get( "Delete profile" ) );

my $button_profile_apply = Gtk2::Button->new;
$button_profile_apply->signal_connect( 'clicked', \&evt_apply_profile, $combobox_settings_profiles );
$button_profile_apply->set_image( Gtk2::Image->new_from_stock( 'gtk-apply', 'button' ) );
my $tooltip_profiles_apply = Gtk2::Tooltips->new;
$tooltip_profiles_apply->set_tip( $button_profile_apply, $d->get( "Set profile for current configuration" ) );
...
$combobox_thumb_operator->insert_text( 0, "Resize" . " (" . $d->get( "Recommended" ) . ")" );
...
$tooltip_thumb_operator->set_tip( $combobox_thumb_operator,
								  $d->get( "Select resize operator\nSee http://www.imagemagick.org/Usage/resize/ for help" ) );
...
$tooltip_thumb->set_tip( $thumbnail_active,
...
my $saveDir_button = Gtk2::FileChooserButton->new( "GScrot - " . $d->get( "Choose folder" ), 'select-folder' );
...
$tooltip_autocopy->set_tip( $autocopy_active, $d->get( "Automatically copy screenshot to clipboard" ) );
...
foreach ( keys %gm_programs ) {
		...
		&& $gm_programs{ $_ }->{ 'name' } ne "" )
   {
	  ...
	  if ( -f $gm_programs{ $_ }->{ 'pixmap' } ) {
		 ...
		 $pixbuf =
			 Gtk2::Gdk::Pixbuf->new_from_file_at_size( "$gscrot_root/share/gscrot/resources/icons/executable.svg", Gtk2::IconSize->lookup( 'menu' ) );
	  }
...
$file_vbox->pack_start( $filetype_box, TRUE, TRUE, 5 );
$file_frame->add( $file_vbox );

$save_vbox->pack_start( $filename_box, TRUE, TRUE, 5 );
$save_vbox->pack_start( $saveDir_box,  TRUE, TRUE, 5 );
$save_frame->add( $save_vbox );

$vbox_basic->pack_start( $file_frame, TRUE, TRUE, 5 );
$vbox_basic->pack_start( $save_frame, TRUE, TRUE, 5 );
...
sub evt_take_screenshot {
   ...
   if ( $data eq "raw" || $data eq "tray_raw" ) {
	  ...
	  $screenshot = &fct_gscrot_workspace( $folder, $filename_value, $filetype_value, $quality_value, $delay_value,
		   ...
		   || $data eq "tray_section" )
   {
	  ...
	  $screenshot =
		  &fct_gscrot_window( $folder, $filename_value, $filetype_value, $quality_value, $delay_value, $combobox_border->get_active, $data );
	  ...
	  $screenshot = &fct_gscrot_area( $folder, $filename_value, $filetype_value, $quality_value, $delay_value );
	  ...
	  if ( $screenshot eq "" ) {
		 ...
		 $screenshot = &fct_imagemagick_perform( "get_object", $screenshot_name, 0, $filetype_value );
	  }
	  else {
	  ...
	  $screenshot =
		  &fct_gscrot_window_direct( $folder, $filename_value, $filetype_value, $quality_value, $delay_value, $combobox_border->get_active, $xid );
   }
	  ...
	  $screenshot =
		  &fct_gscrot_workspace( $folder, $filename_value, $filetype_value, $quality_value, $delay_value, $combobox_border->get_active,
								 $selected_workspace );
   }
   ...
   system( "xset b on" )
	   if $boff_cparam;    #turns on the speaker again if set as arg
   ...
   unless ( $screenshot ) {
	  ...
	  if ( $thumbnail_active->get_active ) {
		 ...
		 unless ( &fct_file_exists( $screenshot_thumbnail_name ) ) {
			&dlg_error_message( $d - get( "Could not generate thumbnail" ) );
   ...
   if ( &fct_file_exists( $screenshot_name ) ) {
	  ...
	  my $new_key_screenshot_thumbnail = &fct_integrate_screenshot_in_notebook( $screenshot_thumbnail_name )
		  if $thumbnail_active->get_active;

	  #autocopy to clipboard if configured
	  if ( $autocopy_active->get_active() ) {
		 my $pixbuf = Gtk2::Gdk::Pixbuf->new_from_file( $session_screens{ $new_key_screenshot }->{ 'long' } );
	  ...
	  if ( $progname_active->get_active ) {
		 ...
		 unless ( $progname_value =~ /[a-zA-Z0-9]+/ ) {
			&dlg_error_message( $d->get( "No application specified to open the screenshot" ) );
			&fct_set_toolbar_sensitive( TRUE );
			return FALSE;
		 }
		 system( "$progname_value '$session_screens{$new_key_screenshot}->{'long'}' &" );    #open picture in external program
	  }
...
sub evt_delete_window {
   ...
   if ( $save_at_close_active->get_active ) {
	  ...
	  &fct_save_settings( $combobox_settings_profiles->get_active_text ) if $combobox_settings_profiles->get_active != -1;
   }
...
sub evt_translate {
   &fct_gnome_open( undef, "https://translations.launchpad.net/gscrot", undef );
}
...
sub evt_in_tab {
   ...
   if ( $key =~ m/^print\[/ ) {
	  ...
	  unless ( &fct_create_thumbnail_and_fileinfos( $session_screens{ $key }->{ 'filename' }, $key ) ) {
		 &dlg_status_message( 1, $session_screens{ $key }->{ 'filename' } . " " . $d->get( "not found" ) );

		 return 0;
	  }
	  my $current_file = quotemeta $session_screens{ $key }->{ 'long' };
	  system( "gtklp $current_file &" );
	  &dlg_status_message( 1, $session_screens{ $key }->{ 'filename' } . " " . $d->get( "will be printed" ) );
   }
   if ( $key =~ m/^delete\[/ ) {
	  $key =~ s/^delete//;
	  unless ( &fct_create_thumbnail_and_fileinfos( $session_screens{ $key }->{ 'filename' }, $key ) ) {
	  ...
	  &dlg_status_message( 1, $session_screens{ $key }->{ 'filename' } . " " . $d->get( "deleted" ) )
   ...
   if ( $key =~ m/^remove\[/ ) {
	  ...
	  &dlg_status_message( 1, $session_screens{ $key }->{ 'filename' } . " " . $d->get( "removed from session" ) )
		  if defined( $session_screens{ $key }->{ 'filename' } );
	  delete( $session_screens{ $key } );                                                            # delete from hash
	  &fct_update_first_tab();
	  $window->show_all;
   ...
   if ( $key =~ m/^reopen\[/ ) {
	  ...
	  &dlg_status_message( 1, $session_screens{ $key }->{ 'filename' } . " " . $d->get( "opened with" ) . " " . $progname_value );
   ...
   if ( $key =~ m/^upload\[/ ) {
	  ...
	  &dlg_account_chooser_and_upload( $session_screens{ $key }->{ 'filename' } );
   }
   if ( $key =~ m/^draw\[/ ) {
	  $key =~ s/^draw//;
	  unless ( &fct_create_thumbnail_and_fileinfos( $session_screens{ $key }->{ 'filename' }, $key ) ) {
	  ...
	  &fct_start_drawing( $session_screens{ $key }->{ 'long' },
						  $session_screens{ $key }->{ 'width' },
						  $session_screens{ $key }->{ 'height' },
						  $session_screens{ $key }->{ 'filetype' }, $d );
	  &fct_update_tab( $key );
   }
   if ( $key =~ m/^rename\[/ ) {
	  $key =~ s/^rename//;
	  ...
	  &dlg_status_message( 1, $session_screens{ $key }->{ 'filename' } . " " . $d->get( "renamed" ) )
		  if &dlg_rename( $session_screens{ $key }->{ 'filename' }, $key );
	  &fct_update_tab( $key );
   }
   if ( $key =~ m/^plugin\[/ ) {
	  $key =~ s/^plugin//;
...
sub evt_settings {
   ...
   if ( $data eq "menu_open" ) {
	  ...
	  if ( $fs_resp eq "accept" ) {
		 ...
		 &fct_integrate_screenshot_in_notebook( $new_file );
		 $fs->destroy();
		 return TRUE;
	  }
	  else {
...
sub evt_delete_profile {
   ...
   if ( $combobox_settings_profiles->get_active_text ) {
	  ...
	  unless (    &fct_file_exists( "$ENV{'HOME'}/.gscrot/profiles/" . $active_text . ".xml" )
			   || &fct_file_exists( "$ENV{'HOME'}/.gscrot/profiles/" . $active_text . "_accounts.xml" ) )
	  {
...
sub evt_apply_profile {
   my ( $widget, $combobox_settings_profiles ) = @_;
   if ( $combobox_settings_profiles->get_active_text ) {
	  &fct_load_settings( undef, 'profile_load', $combobox_settings_profiles->get_active_text );
   }
   return TRUE;
}
...
sub fct_create_session_notebook {
   ...
   my $thumb_first_icon =
	   Gtk2::Image->new_from_pixbuf(
	   Gtk2::Gdk::Pixbuf->new_from_file_at_scale( "$gscrot_root/share/gscrot/resources/icons/session.svg", Gtk2::IconSize->lookup( 'menu' ), TRUE ) );
...
sub fct_integrate_screenshot_in_notebook {
   ...
   $session_screens{ $key }->{ 'tab_icon' } =
	   Gtk2::Image->new_from_pixbuf( $session_screens{ $key }->{ 'thumb' }->scale_simple( Gtk2::IconSize->lookup( 'menu' ), 'tiles' ) );
...
sub fct_set_toolbar_sensitive {
   ...
   $button_web_menu->set_sensitive( $set );
...
sub fct_create_tab {
   ...
   my $button_remove = Gtk2::ToolButton->new( Gtk2::Image->new_from_icon_name( 'gtk-close', 'button' ), undef );
   $button_remove->signal_connect( clicked => \&evt_in_tab, 'remove' . $key );
   my $tooltip_remove = Gtk2::Tooltips->new;
   $tooltip_remove->set_tip( $button_remove, $d->get( "Remove file(s) from session" ) );

   my $button_delete = Gtk2::ToolButton->new( Gtk2::Image->new_from_icon_name( 'gtk-delete', 'button' ), undef );
   $button_delete->signal_connect( clicked => \&evt_in_tab, 'delete' . $key );
   my $tooltip_delete = Gtk2::Tooltips->new;
   $tooltip_delete->set_tip( $button_delete, $d->get( "Delete file(s)" ) );

   my $button_clipboard = Gtk2::ToolButton->new( Gtk2::Image->new_from_icon_name( 'gtk-copy', 'button' ), undef );
   $button_clipboard->signal_connect( clicked => \&evt_in_tab, 'clipboard' . $key );
   my $tooltip_clipboard = Gtk2::Tooltips->new;
   $tooltip_clipboard->set_tip( $button_clipboard, $d->get( "Copy file(s) to clipboard" ) );

   my $button_reopen = Gtk2::ToolButton->new( Gtk2::Image->new_from_icon_name( 'gtk-redo-ltr', 'button' ), undef );
   $button_reopen->signal_connect( clicked => \&evt_in_tab, 'reopen' . $key );
   my $tooltip_reopen = Gtk2::Tooltips->new;
   $tooltip_reopen->set_tip( $button_reopen, $d->get( "Open file(s)" ) );

   my $button_upload = Gtk2::ToolButton->new( Gtk2::Image->new_from_icon_name( 'gtk-go-up', 'button' ), undef );
   $button_upload->signal_connect( clicked => \&evt_in_tab, 'upload' . $key );
   my $tooltip_upload = Gtk2::Tooltips->new;
   $tooltip_upload->set_tip( $button_upload, $d->get( "Upload file to hosting-site" ) );

   my $button_rename = Gtk2::ToolButton->new( Gtk2::Image->new_from_icon_name( 'gtk-edit', 'button' ), undef );
   $button_rename->signal_connect( clicked => \&evt_in_tab, 'rename' . $key );
   my $tooltip_rename = Gtk2::Tooltips->new;
   $tooltip_rename->set_tip( $button_rename, $d->get( "Rename file" ) );

   my $button_plugin = Gtk2::ToolButton->new( Gtk2::Image->new_from_icon_name( 'gtk-execute', 'button' ), undef );
   $button_plugin->signal_connect( clicked => \&evt_in_tab, 'plugin' . $key );
   ...
   my $button_print = Gtk2::ToolButton->new( Gtk2::Image->new_from_icon_name( 'gtk-print', 'button' ), undef );
   $button_print->signal_connect( clicked => \&evt_in_tab, 'print' . $key );
   ...
   unless ( $is_all ) {
	  ...
	  $session_screens{ $key }->{ 'tooltip_filename_tab' }
		  ->set_tip( $session_screens{ $key }->{ 'filename_label' }, $session_screens{ $key }->{ 'filename' } );
	  ...
	  $session_screens{ $key }->{ 'geometry_label' } =
		  Gtk2::Label->new( $session_screens{ $key }->{ 'width' } . "x" . $session_screens{ $key }->{ 'height' } );
	  ...
	  $session_screens{ $key }->{ 'image' }->signal_connect_after( 'expose-event', \&fct_resize_thumb, $key )
		  unless ( $is_all );
	  ...
	  foreach ( keys %plugins ) {
		 if ( $plugins{ $_ }->{ 'menu' } ) {
			...
			my $data = [ $plugins{ $_ }->{ 'binary' }, $plugins{ $_ }->{ 'name' }, $plugins{ $_ }->{ 'lang' }, $key, undef ];
			...
			$btn->set_sensitive( FALSE ) unless $plugins{ $_ }->{ 'ext' } =~ /$session_screens{$key}->{'filetype'}/;
		 }
	  }
	  ...
	  foreach ( keys %{ $session_start_screen{ 'first_page' } } ) {
		 $session_start_screen{ 'first_page' }->{ $_ }->set_sensitive( FALSE ) if $session_start_screen{ 'first_page' }->{ $_ } =~ /button/i;
	  }
...
sub fct_resize_thumb {
   ...
   if ( defined $widget ) {
	  ...
	  {
		 $image->set_from_pixbuf( $thumb->scale_simple( $image->{ 'current_width' }, $image->{ 'current_height' }, 'tiles' ) );
	  }
   }
   return FALSE;
}
...
sub fct_save_settings {
   ...
   if ( defined $profilename ) {
	  $settingsfile = "$ENV{ HOME }/.gscrot/profiles/$profilename.xml" if ( $profilename ne "" );
   }

   #accounts file
   my $accountsfile = "$ENV{ HOME }/.gscrot/accounts.xml";
   if ( defined $profilename ) {
	  $accountsfile = "$ENV{ HOME }/.gscrot/profiles/$profilename\_accounts.xml" if ( $profilename ne "" );
   }
   ...
   #plugins
   foreach ( keys %plugins ) {
	  $settings{ 'plugins' }->{ $_ }->{ 'menu' } = $plugins{ $_ }->{ 'menu' } if defined $plugins{ $_ }->{ 'menu' };
   }
   ...
   open( ACC_FILE, ">$accountsfile" )
	   or &dlg_error_message( $d->get( "Account-settings could not be saved!" ) );
...
sub fct_save_bindings {
   ...
   if ( $keybinding_sel_active->get_active() ) {
	  $client->set( $command_sel, { type => 'string', value => "$gscrot_path --window", } );
	  $client->set(
					$shortcut_sel,
					{
					   type  => 'string',
...
sub fct_load_settings {
   ...
   #settings file
   my $settingsfile = "$ENV{ HOME }/.gscrot/settings.xml";
   $settingsfile = "$ENV{ HOME }/.gscrot/profiles/$profilename.xml" if ( defined $profilename );
   ...
   foreach ( keys %plugins ) {
	  if ( defined $settings_xml->{ 'plugins' }->{ $_ } ) {
		 $plugins{ $_ }->{ 'menu' } = $settings_xml->{ 'plugins' }->{ $_ }->{ 'menu' };
	  }
   }
...
sub fct_load_accounts {
   ...
   #accounts file
   my $accountsfile = "$ENV{ HOME }/.gscrot/accounts.xml";
   $accountsfile = "$ENV{ HOME }/.gscrot/profiles/$profilename\_accounts.xml" if ( defined $profilename );

   my $accounts_xml = XMLin( $accountsfile )
...
sub fct_execute_plugin {
   ...
   if ( $plugin_lang eq "perl" ) {
	  ...
	  my $pid = fork;
	  if ( $pid < 0 ) {
		 &dlg_error_message( $d->get( "Could not execute plugin" ) . ": " . $plugin_name );
	  }
	  if ( $pid == 0 ) {
		 exec(
			sprintf(
...
sub fct_update_first_tab {
   ...
   foreach ( sort keys %session_screens ) {
	  $session_start_screen{ 'first_page' }->{ 'model' }->set(
															   ...
															   $session_screens{ $_ }->{ 'thumb' }->scale_simple( 48, 48, 'tiles' ), 1,
   ...
   $session_start_screen{ 'first_page' }->{ 'size_counter' }
	   ->set_text( $d->get( "Total size" ) . ": " . sprintf( "%.2f", $total_size / 1024 ) . " KB" );
...
sub fct_update_tab {
   ...
   if ( &fct_create_thumbnail_and_fileinfos( $session_screens{ $key }->{ 'filename' }, $key ) ) {
	  ...
	  $session_screens{ $key }->{ 'tab_icon' }
		  ->set_from_pixbuf( $session_screens{ $key }->{ 'thumb' }->scale_simple( Gtk2::IconSize->lookup( 'menu' ), 'tiles' ) );
	  ...
	  $session_screens{ $key }->{ 'geometry_label' }
		  ->set_text( $session_screens{ $key }->{ 'width' } . "x" . $session_screens{ $key }->{ 'height' } );
   }
   else {

	  #update tab icon - file is not existing anymore, maybe deleted manually
	  $session_screens{ $key }->{ 'thumb' } =
		  Gtk2::Gdk::Pixbuf->new_from_file_at_scale( "$gscrot_root/share/gscrot/resources/icons/Image-missing.svg", 400, 400, TRUE );
	  $session_screens{ $key }->{ 'tab_icon' }
		  ->set_from_pixbuf( $session_screens{ $key }->{ 'thumb' }->scale_simple( Gtk2::IconSize->lookup( 'menu' ), 'tiles' ) );
	  $session_screens{ $key }->{ 'image' }->set_from_pixbuf( $session_screens{ $key }->{ 'thumb' } );

	  foreach ( keys %{ $session_screens{ $key } } ) {
		 $session_screens{ $key }->{ $_ }->set_sensitive( FALSE ) if $session_screens{ $key }->{ $_ } =~ /button/i;
		 $session_screens{ $key }->{ $_ }->set_text( "-" )        if $session_screens{ $key }->{ $_ } =~ /label/i;
	  }
...
sub fct_gscrot_area {
   ...
   my ( $rootxp, $rootyp, $rootwidthp, $rootheightp, $rootdepthp ) = $root->get_geometry;
   my $root_item   = undef;
   ...
   $scwin->set_size_request( 100 * $zoom_size_factor, 100 * $zoom_size_factor );
   ...
   $canvas->set_scroll_region( -10 * $zoom_size_factor,
							   -10 * $zoom_size_factor,
   ...
   my $cursor_pixbuf = Gtk2::Gdk::Pixbuf->new_from_file( "$gscrot_root/share/gscrot/resources/icons/gscrot_cursor.png" );
...
#screens should not update while selecting a region
#	my $pixbuf = Gtk2::Gdk::Pixbuf->get_from_drawable ($root, undef, $rootxp, $rootyp, 0, 0, $rootwidthp, $rootheightp);
#	my %attr = (window_type => 'child', wclass => 'GDK_INPUT_OUTPUT', x => 0, y => 0, width => $rootwidthp, height => $rootheightp, override_redirect => 1, colormap => $gdk_screen->get_default_colormap, visual => $gdk_screen->get_system_visual);
#
#	my $backwindow = Gtk2::Gdk::Window->new ($root, \%attr);
#	my $backgc = Gtk2::Gdk::GC->new ($backwindow, undef);
#	$backgc->set_subwindow ('include-inferiors');
#
#	$backwindow->set_back_pixmap (Gtk2::Gdk::Pixmap->new ($root, $rootwidthp, $rootheightp, $rootdepthp), 0);
#	$backwindow->show;
#
		 ...
		 sub {
			...
			if ( $event->type eq 'key-press' ) {
			   ...
			   if ( $rect_w > 1 ) {
				  ...
				  my $pixbuf = Gtk2::Gdk::Pixbuf->get_from_drawable( $root, undef, $rect_x, $rect_y, 0, 0, $rect_w + 1, $rect_h + 1 );
			   ...
			   if (
					( ( $event->x >= $zoom_window_x - 150 ) && ( $event->x <= ( $zoom_window_x + $zoom_window_width + 150 ) ) )
...
sub fct_gscrot_window {
   my ( $folder, $filename_value, $filetype_value, $quality_value, $delay_value, $border, $mode ) = @_;
...
#looking for a section of a window? keep current window in mind and search for children
			   if ( ( $mode eq "section" || $mode eq "tray_section" )
					&& !$window_selected )
			   {
				  return 0 unless defined $children{ 'last_win' };
				  &fct_query_children( $children{ 'last_win' }{ 'gdk_window' }->XWINDOW, $children{ 'last_win' }{ 'gdk_window' }->XWINDOW );

				  #focus selected window (maybe it is hidden)
				  $children{ 'last_win' }{ 'gdk_window' }->focus( time );
				  Gtk2::Gdk->flush;
				  $window_selected = $children{ 'curr_win' }{ 'gdk_window' };
				  return 1;
			   }
			   ...
			   if ( defined $children{ 'last_win' } ) {
				  ...
				  $children{ 'curr_win' }{ 'width' } -= $children{ 'curr_win' }{ 'x' } + $children{ 'curr_win' }{ 'width' } - $rootwidthp
					  if ( $children{ 'curr_win' }{ 'x' } + $children{ 'curr_win' }{ 'width' } > $rootwidthp );
				  $children{ 'curr_win' }{ 'height' } -= $children{ 'curr_win' }{ 'y' } + $children{ 'curr_win' }{ 'height' } - $rootheightp
					  if ( $children{ 'curr_win' }{ 'y' } + $children{ 'curr_win' }{ 'height' } > $rootheightp );
				  if     ( $children{ 'curr_win' }{ 'x' } < $rootxp ) {
					 $children{ 'curr_win' }{ 'width' } = $children{ 'curr_win' }{ 'width' } + $children{ 'curr_win' }{ 'x' };
					 $children{ 'curr_win' }{ 'x' }     = 0;
				  }
				  if ( $children{ 'curr_win' }{ 'y' } < $rootyp ) {
					 $children{ 'curr_win' }{ 'height' } = $children{ 'curr_win' }{ 'height' } + $children{ 'curr_win' }{ 'y' };
					 $children{ 'curr_win' }{ 'y' }      = 0;
				  }

				  #...take it
				  my $pixbuf = Gtk2::Gdk::Pixbuf->get_from_drawable( $root, undef,
																	 $children{ 'curr_win' }{ 'x' },
																	 $children{ 'curr_win' }{ 'y' },
																	 0, 0,
			   ...
			   unless ( $window_selected ) {
				  ...
				  foreach my $curr_window ( @windows ) {
					 ...
					 next
						 if ( $curr_window->get_xid == $window->window->get_xid && $is_in_tray );
...
#							if($curr_window->is_visible_on_workspace ($screen->get_active_workspace)){
					 ...
					 print "Current Event x: " . $event->x . ", y: " . $event->y . "\n"
						 if $debug_cparam;
					 if (
						  $curr_window->is_visible_on_workspace( $screen->get_active_workspace )
						  && (    ( $event->x >= $xp )
							   && ( $event->x <= ( $xp + $widthp ) ) )
						  && (    ( $event->y >= $yp )
							   && ( $event->y <= ( $yp + $heightp ) ) )
						)
					 {
						if ( ( $xp + $widthp ) * ( $yp + $heightp ) <= $min_x * $min_y ) {
						   print "Parent X: $xp, Y: $yp, Width: $widthp, Height: $heightp\n"
							   if $debug_cparam;
						   $children{ 'curr_win' }{ 'window' }     = $curr_window;
						   $children{ 'curr_win' }{ 'gdk_window' } = $drawable;
						   $children{ 'curr_win' }{ 'x' }          = $xp;
						   $children{ 'curr_win' }{ 'y' }          = $yp;
						   $children{ 'curr_win' }{ 'width' }      = $widthp;
						   $children{ 'curr_win' }{ 'height' }     = $heightp;
						   $min_x                                  = $xp + $widthp;
						   $min_y                                  = $yp + $heightp;
						}
					 }
				  ...
				  if ( defined $children{ 'curr_win' } ) {
					 print "Currently smallest window: " . $children{ 'curr_win' }{ 'window' }->get_name . "\n"
						 if $debug_cparam;
					 print
						  ...
						  and $window_selected )
					 {
						print "QueryPointer for current window...\n"
							if $debug_cparam;
						if ( defined $children{ 'curr_win' }{ 'gdk_window' } ) {

						   #selected window is parent
						   my $curr_parent = $window_selected->XWINDOW;
						   foreach my $curr_child ( keys %{ $children{ $curr_parent } } ) {
							  next unless defined $curr_child;
							  print "Child Current Event x: " . $event->x . ", y: " . $event->y . "\n"
								  if $debug_cparam;
							  if (
								   (
									 ( $event->x >= $children{ $curr_parent }{ $curr_child }{ 'x' } )
									 && ( $event->x <=
										  ( $children{ $curr_parent }{ $curr_child }{ 'x' } + $children{ $curr_parent }{ $curr_child }{ 'width' } ) )
								   )
								   && (
									   ( $event->y >= $children{ $curr_parent }{ $curr_child }{ 'y' } )
									   && ( $event->y <=
										  ( $children{ $curr_parent }{ $curr_child }{ 'y' } + $children{ $curr_parent }{ $curr_child }{ 'height' } ) )
									  )
								 )
							  {
								 if ( ( $children{ $curr_parent }{ $curr_child }{ 'x' } + $children{ $curr_parent }{ $curr_child }{ 'width' } ) *
									  ( $children{ $curr_parent }{ $curr_child }{ 'y' } + $children{ $curr_parent }{ $curr_child }{ 'height' } ) <=
									  $min_x *
									  $min_y )
								 {
									$children{ 'curr_win' }{ 'gdk_window' } = $children{ $curr_parent }{ $curr_child }{ 'gdk_window' };
									$children{ 'curr_win' }{ 'x' }          = $children{ $curr_parent }{ $curr_child }{ 'x' };
									$children{ 'curr_win' }{ 'y' }          = $children{ $curr_parent }{ $curr_child }{ 'y' };
									$children{ 'curr_win' }{ 'width' }      = $children{ $curr_parent }{ $curr_child }{ 'width' };
									$children{ 'curr_win' }{ 'height' }     = $children{ $curr_parent }{ $curr_child }{ 'height' };
									$min_x = $children{ $curr_parent }{ $curr_child }{ 'x' } + $children{ $curr_parent }{ $curr_child }{ 'width' };
									$min_y = $children{ $curr_parent }{ $curr_child }{ 'y' } + $children{ $curr_parent }{ $curr_child }{ 'height' };
								 }
							  }
						   }
						}
			   ...
			   #draw rect if needed
			   if ( $last_selected_window ne $children{ 'curr_win' }{ 'gdk_window' } ) {
				  ...
				  $last_selected_window                   = $children{ 'curr_win' }{ 'gdk_window' };
				  $last_selected_window_wnck              = $children{ 'curr_win' }{ 'window' };
				  $children{ 'last_win' }{ 'window' }     = $children{ 'curr_win' }{ 'window' };
				  $children{ 'last_win' }{ 'gdk_window' } = $children{ 'curr_win' }{ 'gdk_window' };
				  $children{ 'last_win' }{ 'x' }          = $children{ 'curr_win' }{ 'x' } - 3;
				  $children{ 'last_win' }{ 'y' }          = $children{ 'curr_win' }{ 'y' } - 3;
				  $children{ 'last_win' }{ 'width' }      = $children{ 'curr_win' }{ 'width' } + 5;
				  $children{ 'last_win' }{ 'height' }     = $children{ 'curr_win' }{ 'height' } + 5;
   ...
   foreach ( @qkids ) {
	  ...
	  if ( defined $gdk_window ) {
		 ...
		 foreach my $checkchild ( keys %{ $children{ $xparent } } ) {
			$dub = TRUE
				if $children{ $xparent }{ $checkchild }{ 'gdk_window' } == $gdk_window;
		 }
		 unless ( $dub == TRUE ) {
...
sub fct_gscrot_window_direct {
   my ( $folder, $filename_value, $filetype_value, $quality_value, $delay_value, $border, $curr_xid ) = @_;
   ...
   my $pixbuf = Gtk2::Gdk::Pixbuf->get_from_drawable( $root, undef, $xp, $yp, 0, 0, $widthp, $heightp );
...
sub fct_gscrot_workspace {
   my ( $folder, $filename_value, $filetype_value, $quality_value, $delay_value, $border, $selected_workspace ) = @_;
   $active_workspace = $screen->get_active_workspace;
...
sub fct_create_thumbnail_and_fileinfos {
   ...
   $filename = &fct_switch_home_in_file( $filename );
   $session_screens{ $key }->{ 'thumb' } = Gtk2::Gdk::Pixbuf->new_from_file_at_scale( $filename, 400, 400, TRUE );
   $session_screens{ $key }->{ 'thumb' } =
	   Gtk2::Gdk::Pixbuf->new_from_file_at_scale( "$gscrot_root/share/gscrot/resources/icons/Image-missing.svg", 400, 400, TRUE )
...
sub fct_ret_workspace_menu {
   ...
   foreach my $space ( @workspaces ) {
	  ...
	  if ( $active_workspace->get_number != $space->get_number ) {
		 ...
		 $wrkspace_item->signal_connect( 'activate' => \&evt_take_screenshot,
...
sub fct_load_plugin_tree {
   ...
   $effects_model = Gtk2::ListStore->new(
										  'Gtk2::Gdk::Pixbuf', 'Glib::String', 'Glib::Boolean', 'Glib::String',
   ...
   foreach ( sort keys %plugins ) {
	  if ( $plugins{ $_ }->{ 'binary' } ne "" ) {
		 ...
		 if ( -f $plugins{ $_ }->{ 'pixbuf' } ) {
			$pixbuf = Gtk2::Gdk::Pixbuf->new_from_file_at_size( $plugins{ $_ }->{ 'pixbuf' }, Gtk2::IconSize->lookup( 'menu' ) );
		 }
		 else {
			$pixbuf =
				Gtk2::Gdk::Pixbuf->new_from_file_at_size( "$gscrot_root/share/gscrot/resources/icons/executable.svg",
														  Gtk2::IconSize->lookup( 'menu' ) );
		 }
		 ...
		 $plugins{ $_ }->{ 'lang' } = "shell" if $plugins{ $_ }->{ 'lang' } eq "";
		 chomp( $plugins{ $_ }->{ 'name' } );
		 ...
		 $effects_model->set(
							  $effects_model->append,        0, $pixbuf,                      1, $plugins{ $_ }->{ 'name' },     2,
							  $plugins{ $_ }->{ 'menu' },    3, $plugins{ $_ }->{ 'binary' }, 4, $plugins{ $_ }->{ 'category' }, 5,
							  $plugins{ $_ }->{ 'tooltip' }, 6, $pixbuf_jpeg,                 7, $pixbuf_png,                    8,
							  $plugins{ $_ }->{ 'lang' },    9, $_,
							);
	  }
	  else {
...
sub fct_set_model_plugins {
   ...
   #icon
   $effects_tree->set_tooltip_column( 5 ) if Gtk2->CHECK_VERSION( 2, 11, 0 );
   my $tv_clmn_pix_text = Gtk2::TreeViewColumn->new;
...
#menu
   ...
   $renderer_toolbar_effects->signal_connect( toggled => \&evt_plugins_toolbar, $effects_model );
   $tv_clmn_toolbar_text->set_attributes( $renderer_toolbar_effects, 'active' => 2 );
...
#category
   ...
   $tv_clmn_category_text->set_attributes( $renderer_category_effects, text => 4 );
...
sub fct_init {
   ...
   my @plugins = glob( "$gscrot_root/share/gscrot/resources/system/plugins/*/*" );
   foreach ( @plugins ) {
	  if ( -d $_ ) {
		 my $dir_name = $_;
		 $dir_name =~ s{^.*/}{};
		 $plugins{ $_ }->{ 'binary' } = "$_/$dir_name"
			 if ( -f "$_/$dir_name" && -r "$_/$dir_name" );
		 $plugins{ $_ }->{ 'pixbuf' } = "$_/$dir_name.png"
			 if ( -f "$_/$dir_name.png" && -r "$_/$dir_name.png" );

		 #we prefer svgs
		 $plugins{ $_ }->{ 'pixbuf' } = "$_/$dir_name.svg"
			 if ( -f "$_/$dir_name.svg" && -r "$_/$dir_name.svg" );
	  }
   }
...
sub dlg_error_message {
   ...
   my $error_dialog = Gtk2::MessageDialog->new( $window, [ qw/modal destroy-with-parent/ ], 'error', 'ok', $dlg_error_message );
...
sub dlg_question_message {
   ...
   my $question_dialog = Gtk2::MessageDialog->new( $window, [ qw/modal destroy-with-parent/ ], 'question', 'yes_no', $dlg_question_message );
...
sub dlg_rename {
   ...
   if ( $input_response eq "accept" ) {
	  ...
	  $dlg_rename_text =~ s/$old_file_name/$new_name/;
	  unless ( $old_file_name_full eq $dlg_rename_text ) {    #filenames eq? -> nothing to do here
		 unless ( &fct_file_exists( $dlg_rename_text ) ) {
			rename( &fct_switch_home_in_file( $old_file_name_full ), &fct_switch_home_in_file( $dlg_rename_text ) );
		 }
		 else {
			&dlg_error_message( $d->get( "File already exists,\nplease choose an alternative filename!" ) );
...
sub dlg_plugin {
   ...
   foreach ( sort keys %plugins ) {
	  ...
	  if ( $plugins{ $_ }->{ 'binary' } ne "" ) {
		 ...
		 utf8::decode $plugins{ $_ }->{ 'name' };
		 chomp( $plugins{ $_ }->{ 'name' } );
		 $model->set( $model->append, 0, $pixbuf, 1, $plugins{ $_ }->{ 'name' }, 2, $plugins{ $_ }->{ 'binary' }, 3, $plugins{ $_ }->{ 'lang' } );
	  }
	  else {
		 print "WARNING: Program $_ is not configured properly, ignoring\n";
	  }
...
sub dlg_account_chooser_and_upload {
   ...
   my $hosting_dialog = Gtk2::Dialog->new(
										   $dlg_header,
										   $window,
										   [ qw/modal destroy-with-parent/ ],
										   'gtk-ok'     => 'accept',
   ...
   foreach ( keys %accounts ) {
	  $model->set( $model->append, 0, $accounts{ $_ }->{ 'host' }, 1, $accounts{ $_ }->{ 'username' }, 2, $accounts{ $_ }->{ 'password' } )
   ...
   if ( $hosting_response eq "accept" ) {
	  ...
	  if ( $hosting_host eq "ubuntu-pics.de" ) {
		 my %upload_response;
		 %upload_response = &fct_upload_ubuntu_pics( &fct_switch_home_in_file( $file_to_upload ),
													 $hosting_username, $hosting_password, $debug_cparam, GSCROT_VERSION );
		 if ( is_success( $upload_response{ 'status' } ) ) {
			...
			&dlg_status_message( 1, $file_to_upload . " " . $d->get( "uploaded" ) );
		 }
		 else {
		 ...
		 my %upload_response;
		 %upload_response = &fct_upload_imagebanana( &fct_switch_home_in_file( $file_to_upload ),
													 $hosting_username, $hosting_password, $debug_cparam, GSCROT_VERSION );
		 if ( is_success( $upload_response{ 'status' } ) ) {
...
sub dlg_info_message {
   ...
   my $info_dialog = Gtk2::MessageDialog->new( $window, [ qw/modal destroy-with-parent/ ], 'info', 'ok', $dlg_info_message );
...
sub dlg_profile_name {
   ...
   $new_profile_name->set_text( $curr_profile_name ) if defined $curr_profile_name;
   ...
   if ( $profile_response eq "accept" ) {
	  ...
	  if ( &fct_file_exists( "$ENV{'HOME'}/.gscrot/profiles/$entered_name.xml" ) ) {

		 if ( &dlg_question_message( $d->get( "Profile already exists.\nDo you want to overwrite the existing profile?" ) ) ) {
